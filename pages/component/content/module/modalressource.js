...
export default function ModalRessource({
  data,
  openP,
  updater,
  token,
  ...
  useEffect(() => {
    console.log("ModalRessource",data)
    setOpen(openP);
  }, [openP, updater, save]);
  ...
  const handleCloseM = (event, reason) => {
    console.log("handleCloseM", reason);
    if ((reason && reason == "backdropClick") || reason == "escapeKeyDown")
      return;
    //console.log("data",data)
    if (data.type > 3) {
      if (!save) setOpenAlert(true);
      else setOpen(false);
    } else setOpen(false);
  };
  ...
  return (
    <div>
      ...
      >
        <DialogTitle>{data.titre}  {data.maxTry!=undefined?" - Consultations restantes :"+(data.maxTry-data.try).toString():""} </DialogTitle>
