...
export default function Live({module,token}){
    ...
    const [clearQL, setClearQL] = React.useState(false);
  ...
  <Grid item xs={8}>
    <Item sx={{height: "100%" }}>
    <TextField id="standard-basic" label="Titre"  sx={{width:"90%"}} 
          value={titre}
          onChange={(event) => {
  ...
  <Grid item xs={3}>
    <Item  sx={{height: "100%"}}>
    <Typography component="span">Public :
    <Switch
    ...
    /> 
     ...
     {
       publicLive?<Typography component="span"> lien :<IconButton disabled={socket==undefined} component="span"> <ContentCopyIcon onClick={() => {navigator.clipboard.writeText(window.location.href+"live/"+publicKey)}}  /> </IconButton> </Typography> :<> </>
     }
     {
       publicLive?<Typography component="span">  :<IconButton disabled={socket==undefined} component="span"> <QrCode2Icon onClick={() => {setModalQrCode(true)}}  /> </IconButton> </Typography> :<> </>
     }
  ...
  <Grid item xs={1}>
    <Item  sx={{height: "100%"}}>
    {
             socket==undefined?<Button variant="outlined"  onClick={handleStart} sx={{width:"10%"}}>Start</Button>:<StreamIcon sx={{ color: green[500] }} />
           }
    </Item>
  </Grid>
  </Grid>
    ...
    <DialogContentText id="alert-dialog-description">
      ...
      <Stack spacing={3}>
        ...
        <Box sx={{m:2}}>
          ...
          <Box
      sx={{ width: '100%', height: 300, maxWidth: "100%", bgcolor: 'background.paper' }}
    >
       <List
      sx={{
        width: '100%',
