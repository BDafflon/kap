...
async function editionRessource(
  ...
  const response = await fetch(ConfigData.SERVER_URL + url, requestOptions);
...
export default function CreationRessource({
  ...
  if (ressourceIdEdition == undefined) {
    ...
    ressourceIdEdition.dateF =
      new Date(
        newValue.getFullYear(),
        newValue.getMonth(),
        newValue.getDate()
      ) / 1000;
    ressourceIdEdition.dateO =
      new Date(
        newValue.getFullYear(),
        newValue.getMonth(),
        newValue.getDate()
    ...
    if (ressourceIdEdition.code > 3) {
      for (var i = 0; i < ressourceIdEdition.content.length; i++) {
        ressourceIdEdition.content[i]["iddb"] =
          ressourceIdEdition.content["id"];
  ...
  const [maxTry, setMaxTry] = React.useState(0);
  ...
  const handleSubmit = async (event) => {
    ...
    var nbQ =
      localStorage.getItem("nbQuestion") == null
        ? 0
        : localStorage.getItem("nbQuestion");
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={4}>
              ...
              <TextField
                id="outlined-number"
                value={maxTry}
                label="Nombre de consolutation maximum (0:infini)?"
                onChange={(event) => {
