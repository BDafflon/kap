...
async function sendReponseList(data,dataReponse,reponseList,questionsListe,token){
    var d = []
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({"questionList":d,"ressource":data,"dataReponse":dataReponse,"reponseList":reponseList}),
    };
...
function GetForm({data,handleChild, index, token, type, correction}){
    ...
    if (type == '0') {
        return <QuestionRapideForm data={data} handleChild={handleChild} index={index} type={type} correction={correction} />
      }
      if (type == '1') {
        return <QuestionLongueForm data={data}  handleChild={handleChild} index={index} type={type} correction={correction}/>
      }
      if (type == '2') {
        return <QuestionQCMForm data={data} handleChild={handleChild} index={index} type={type} correction={correction}/>
      }
      if (type == '3') { 
        return <QuestionListForm data={data}  handleChild={handleChild} index={index} type={type} correction={correction}/>
      }
      if (type == '4') {
        return <QuestionFileForm data={data}  handleChild={handleChild} index={index} token={token} type={type}  correction={correction}/>
      }
      if (data.type == '6') {
        return <QuestionSansQuestionForm data={data}  handleChild={handleChild} index={index} token={token} type={type}/>
      }
...
export default function CorrectionContent ({ dataQuestion, dataReponse, handleClose, token,handleNextCorrection}) {
    ...
    const handleNext = async () => {
        ...
        if(activeStep === questionsListe.length - 1){
            ...
            await sendReponseList(dataQuestion,dataReponse, reponseList, questionsListe, token)
            setActiveStep(0)
        ...
        dataQuestion.questions.forEach(element => {
            if(element.type == -1){
                ...
                dataReponse.reponse.forEach(e => {
                  if(element.id==e.id_question)
                    element.reponseuser=e.reponse
                });
    ...
    const handleChild = (data,index,field,v) => e => {
        ...
        if(field=="note")
        newArr[activeStep].questions[index].note=v
        }
        console.log("handleChild - questionsListe",newArr[activeStep].questions[index].note)
        ...
        console.log("handleChild - questionsListe",questionsListe[activeStep].questions[index].note)
      }
...
<Box sx={{ width: '100%' }}>
        ...
        <React.Fragment>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
            ...
            <Button onClick={handleNext}>
              {activeStep === questionsListe.length - 1 ? "Fermer" : 'Suivant'}
            </Button>
          </Box>
        </React.Fragment>
      )}
    ...
    questionsListe[activeStep].questions.map((page, index) => (
             ...
             <Box sx={{ my: 2 }}>
                <GetForm data={page} handleChild={handleChild} index={index} token={token} correction={true} type={page.type} />
            </Box>
    ...
    {
        activeStep === questionsListe.length - 1  ? 'Enregistrer la correction ' : ''
    }
    {
        activeStep === questionsListe.length - 1 ? <Button onClick={handleNext}> Envoyer </Button> : ''
    }
