...
async function sendQuestionsList(data,questionsListe,token,timeLeft){
    ...
    console.log("sendQuestionsList stringify",JSON.stringify({"questionList":d,"ressource":data}))
...
function GetForm({data,handleChild, index, token, type, correction,autoCorrection}){
    console.log("GetForm",data)
    if (data.type == '0') {
        return <QuestionRapideForm data={data} handleChild={handleChild} index={index} type={type} autoCorrection={autoCorrection} correction={correction} />
      }
      if (data.type == '1') {
        return <QuestionLongueForm data={data}  handleChild={handleChild} index={index} type={type} autoCorrection={autoCorrection} correction={correction}/>
      }
      if (data.type == '2') {
        return <QuestionQCMForm data={data} handleChild={handleChild} index={index} type={type} autoCorrection={autoCorrection} correction={correction}/>
      }
      if (data.type == '3') { 
        return <QuestionListForm data={data}  handleChild={handleChild} index={index} type={type} autoCorrection={autoCorrection} correction={correction}/>
      }
      if (data.type == '4') {
        return <QuestionFileForm data={data}  handleChild={handleChild} index={index} token={token} autoCorrection={autoCorrection} type={type}/>
      }

      if (data.type == '6') {
        return <QuestionSansQuestionForm data={data}  handleChild={handleChild} index={index} token={token} autoCorrection={autoCorrection} type={type}/>
      }
...
function getSeverity(data,timer){
  if (data.timer=="") return "info"
  if(data.timer!="" && parseInt(timer.split(":")[0]==0) && parseInt(timer.split(":")[1])<0) return "error"
  if(data.timer!="" && parseInt(timer.split(":")[0]==0) && parseInt(timer.split(":")[1])<10) return "warning"
}
export default function FormContent ({ data, handleClose, token}) {
    ...
    const [timeStart, setTimeStart] = React.useState(Math.round(new Date()/1000))
    ...
    useEffect(() => {
        ...
        if(data.timer!= undefined){
          ...
          interval = setInterval(() => setTimeLeft(getTimer(timeStart,data.timer)), 1000);
        }
        var pages=[]
        ...
        data.content.forEach(element => {
          if(data.type==6){
            if(element.type==2){ // QCM
              element.tabChoix = element.choix.split("§")
              element.tabRep = Array.apply(null, Array(element.tabChoix.length)).map(function (x, i) { return false; })
          }
          if(element.type==3){ // Liste a ordonner 
              element.tabChoix = element.reponse.split("§")
              element.tabRep = Array.apply(null, Array(element.tabChoix.length)).map(function (x, i) { return ""; })
          }
    ...
    const handleChild = (data,index,type,field) => e => {
        ...
        if(type=="QCM"){
            ...
            questionsListe[activeStep].questions[index].tabRep[field]=e.target.checked
...
<Box sx={{ width: '100%' }}>
        ...
        <React.Fragment>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
            ...
            <Button onClick={handleNext}>
              {activeStep === questionsListe.length - 1 ? correction?"Fermer":'Envoyer' : 'Suivant'}
            </Button>
          </Box>
        </React.Fragment>
      )}
    ...
    questionsListe[activeStep].questions.map((page, index) => (
             ...
             <Box sx={{ my: 2 }}>
                <GetForm data={page} handleChild={handleChild} index={index} token={token} autoCorrection={data.type==6} correction={correction} type={data.type}/>
            </Box>
    ...
    {
        activeStep === questionsListe.length - 1 && !correction ? 'Valider votre evaluation en cliquant sur ' : ''
    }
    {
        activeStep === questionsListe.length - 1 && !correction? <Button onClick={handleNext}> Envoyer </Button> : ''
    }
