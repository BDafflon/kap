...
export default function CardActivity({data,token}){
    ...
    useEffect(() => {
      const fetchData = async () => {
        if(data!= undefined){
          ...
          var rep = await getActivity(data,token)   
          setList(rep)    
        } 
          ...
          <ListSubheader component="div" id="nested-list-subheader">
        ...
        }>
            {
                listActivity.map((item, index) => (
                    ...
                    <Box >
                      {
                    console.log("item",item.eval==undefined?"":item.eval.note)
                  }
                    <ListItemButton   alignItems="flex-start">
                            <ListItemAvatar>
                              {
                                item.eval==undefined?<Avatar  alt={TypeRessource(item.type_ressource)} src="/static/images/avatar/1.jpg" />:<Avatar sx={{ bgcolor: green[500] }} alt={item.eval.note+""} src="/static/images/avatar/1.jpg" />
                              }
                             
                            </ListItemAvatar>
                            <ListItemText
                            primary={"["+TypeRessource(item.type_ressource)+"]: "+item.titre}
                            secondary={
                                <React.Fragment>
                                <Typography
                                    sx={{ display: 'inline' }}
                                    ...
                                    color="text.primary"
                                >
                                    le {getDate(item.dateO)} : {((item.dateF-item.dateO)/60).toFixed(2)} min
                                </Typography>
                                {" "}
                                </React.Fragment>
                            }
