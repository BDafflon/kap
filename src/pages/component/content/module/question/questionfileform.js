...
export default function QuestionFileForm ({data,handleChild, index, token, correction, autoCorrection}) {
  ...
  const changeHandler = async (event) => {
		...
		setIsSelected(true);
    var rep = await uploadFile("file-"+data.id+"-"+data.id_module,event.target.files[0],token);
  ...
  const handleClose = () => {
    ...
    return(
        ...
        <Box sx={{  p: 2.25, boxShadow: 1 }}>
                  ...
                  <Typography variant='button' gutterBottom> Fichier :  </Typography><Typography variant="button" display="inline" gutterBottom >{isFile?selectedFile.name:""}   {isFile?"("+selectedFile.type+")":""} </Typography>
                  

                  </Box>
                  <Box>
                  <Typography variant='button' gutterBottom> Format autorisés : </Typography><Typography variant="caption" display="inline" gutterBottom >{data.formats}</Typography>
                  </Box><Box>
                  <Typography variant='button' gutterBottom> Taille max :   </Typography><Typography variant="caption" display="inline" gutterBottom >{data.size}Mo</Typography>
                  </Box>
